...
import type { Convidado } from '../pages/Dashboard';

interface Props {
  convidados: Convidado[];
}

type FiltroStatus = 'Todos' | 'Confirmado' | 'Não vai' | 'Pendente';

const RESTRICOES = ['—', 'Vegetariano', 'Vegano', 'Sem glúten', 'Sem lactose', 'Outra'];

export function AbaConvidados({ convidados }: Props) {
  ...
  const totalConfirmados = convidados
    ...
    .reduce((t, c) => t + 1 + c.quantidadeAcompanhantes, 0);
  const totalNaoVao = convidados.filter((c) => c.confirmado === false && c.nomeCompleto).length;
  // "Pendente" = sem resposta ainda (neste modelo, todos têm registro; adapte conforme seu Firestore)
  ...
  const abrirNovoConvidado = () => {
    setEditando(null);
    setForm({ nomeCompleto: '', confirmado: false, quantidadeAcompanhantes: 0, mesa: 1, restricaoAlimentar: '—' });
    setMostrarForm(true);
  };
  ...
  const salvar = async () => {
    if (!form.nomeCompleto.trim()) return alert('Informe o nome do convidado.');
    try {
      if (editando) {
        await updateDoc(doc(db, 'convidados', editando.id), { ...form });
      } else {
        await addDoc(collection(db, 'convidados'), { ...form });
      }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: 'Confirmados', value: totalConfirmados, color: 'emerald' },
          { label: 'Não vão', value: totalNaoVao, color: 'rose' },
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-2 flex-wrap">
          {(['Todos', 'Confirmado', 'Não vai'] as FiltroStatus[]).map((f) => (
            <button key={f} onClick={() => setFiltroStatus(f)} className={filterBtnClass(filtroStatus === f)}>
              {f}
            </button>
          ))}
        </div>
      ...
      {mostrarForm && (
        <div className="bg-rose-50 border border-pink-200 rounded-2xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select
                ...
                className="w-full border border-pink-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-rose-200 bg-white"
              >
                {RESTRICOES.map((r) => <option key={r}>{r}</option>)}
              </select>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="confirmado" className="text-sm text-rose-700">Confirmou presença</label>
          </div>
          <div className="flex gap-2 justify-end">
            <button
              onClick={() => setMostrarForm(false)}
              className="text-pink-400 hover:text-rose-600 text-sm px-4 py-2 rounded-xl border border-pink-100 hover:bg-rose-50 transition"
            >
              Cancelar
            </button>
            <button
          ...
          <tbody className="divide-y divide-pink-50">
            {convidadosFiltrados.map((c) => (
              <tr key={c.id} className="hover:bg-rose-50/30 transition">
                ...
                <td className="p-4 text-rose-700 text-center">{c.mesa ?? '—'}</td>
                <td className="p-4 text-rose-700 text-center">
                  {c.confirmado ? c.quantidadeAcompanhantes : '—'}
                </td>
                <td className="p-4 text-xs text-pink-500">{c.restricaoAlimentar ?? '—'}</td>
                <td className="p-4">
                  <span
                ...
                <td className="p-4">
                  <div className="flex gap-1">
                    <button
                      ...
                      className="text-blue-400 hover:text-blue-600 text-xs px-2 py-1 rounded bg-blue-50 hover:bg-blue-100 transition"
                    >
                      Editar
                    </button>
                    <button
